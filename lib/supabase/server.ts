 ...
 *
 * Next 15 note: `cookies()` became async. We use the official
 * `UnsafeUnwrappedCookies` escape hatch so every caller can keep using
 * `const sb = createClient()` without adding an `await`. This emits a
 * dev-only warning but does NOT break production, and is supported
 * through Next 15.x. When Next 16 arrives this path stops working — at
 * that point we migrate to proper async (cookies = await cookies(),
 * createClient async, await createClient() at every call site).
 */
